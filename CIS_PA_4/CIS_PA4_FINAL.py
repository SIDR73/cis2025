...
def icp_algorithm(d, root, V, Indices):
    """
    Initialize a guess for F_reg and perform iterations with updated guesses till desired convergence occurs. 
    Calculate final s_k and c_k values with transformation.
    """
    #Initialize d as vector Q and get Ns
    Q = d
    Ns = d.shape[0]
    ...
    while True:
        ...
        for k in range(Ns):
            ...
            #Finnd closest point on mesh to sample point 
            c_k, d2_k = search_tree_point(root, V, Indices, s_k, 1)
    ...
    #Compute final answers for sample points using determined F_reg and find closest point on mesh to it 
    for k in range(Ns):
        q_k = Q[k]
        s_adjust_k = frame_transformation(R_reg, q_k, p_reg)
        S_list.append(s_adjust_k)
        c_adjust_k, _ = search_tree_point(root, V, Indices, s_adjust_k, 1)

        C_list.append(c_adjust_k)

    return R_reg, p_reg, np.array(S_list), np.array(C_list), n
...
def process_dataset(data_prefix, extract_path):
    ...
    R_reg, p_reg, ss, cs, n= icp_algorithm(ds,root, V, Indices)
    ...
    print(f"  Completed {data_prefix}: {n} iterations")
